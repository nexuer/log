...
func (h *slogHandler) Handle(ctx context.Context, record slog.Record) error {
	ctx = mergeCallerDepth(ctx, h.ctx)
	if h.lazy {
...
func (h *loggerSlogHandler) Handle(ctx context.Context, record slog.Record) error {
	ctx = mergeCallerDepth(ctx, h.logger.ctx)
	attrs := make([]any, 0, record.NumAttrs())
	record.Attrs(func(attr slog.Attr) bool {
		attrs = append(attrs, attr)
		return true
	})
	return h.logger.Log(ctx, Level(record.Level), record.Message, attrs...)
}
...
func (h *loggerSlogHandler) WithAttrs(attrs []slog.Attr) slog.Handler {
	...
	l2 := h.logger.clone()
	l2.handler = l2.handler.WithFields(context.Background(), fields...)
	return &loggerSlogHandler{logger: l2}
}
